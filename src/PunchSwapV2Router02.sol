...
import './interfaces/IPunchSwapV2Router02.sol';
import './libraries/PunchSwapV2Library.sol';
import './libraries/SafeMath.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWFLOW.sol';

contract PunchSwapV2Router02 is IPunchSwapV2Router02 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IPunchSwapV2Pair(PunchSwapV2Library.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PunchSwapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = PunchSwapV2Library.sortTokens(input, output);
            IPunchSwapV2Pair pair = IPunchSwapV2Pair(PunchSwapV2Library.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PunchSwapV2Library.pairFor(factory, path[0], path[1]), amountIn
        );
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            'PunchSwapV2Router: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
        ...
        ensure(deadline)
    {
        ...
        assert(IWFLOW(WFLOW).transfer(PunchSwapV2Library.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        uint amountOut = IERC20(WFLOW).balanceOf(address(this));
        require(amountOut >= amountOutMin, 'PunchSwapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    {
        return PunchSwapV2Library.getAmountIn(amountOut, reserveIn, reserveOut);
    }
